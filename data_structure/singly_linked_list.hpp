...
public:
    ...
    iterator FindIf (const T &,
                     CompareFunction); //Find which satisfies CompareFunction(a, b)
...
template<typename T>inline  void  SinglyLinkedList<T>::Clear()
{
    Node *p = head_->next;
    while (p)
    {
...
template<typename T>
inline typename
SinglyLinkedList<T>::iterator SinglyLinkedList<T>::GetPrecursor (const T &val)
{
...
template<typename T>
inline typename
SinglyLinkedList<T>::iterator SinglyLinkedList<T>::GetSuccessor (const T &val)
{
    Node *p = head_->next;
    while (p && p->data != val)
    {
...
template<typename T>
inline typename SinglyLinkedList<T>::iterator SinglyLinkedList<T>::Insert (
